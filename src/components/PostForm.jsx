...
function PostForm(props) {
    ...
    const currentPost = (match && match.params && match.params[0]) ? posts.find(post => (parseInt(post.id) === parseInt(match.params[0]))) : {
        id: id,
                    ...
                    <div className="modal-content">
                        ...
                        <div className="modal-footer">
                            ...
                            <button type="button" className="btn btn-primary"
                                    onClick={handleEditFinish}>Сохранить</button>
                            }
                            {(curMode == 'view' && match && match.params[0]) &&
                            <>
                                <button type="button" className="btn btn-primary" onClick={handleEdit}>Изменить</button>
                                <button type="button" className="btn btn-secondary" onClick={handleDelete}>Удалить
                                </button>
                            </>
                            }
                        </div>
